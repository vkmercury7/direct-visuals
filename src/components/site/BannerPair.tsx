  ...
  return (
    <section className="w-full bg-background">
      <div className="mx-auto grid w-full max-w-5xl gap-4 px-4 md:grid-cols-2 md:gap-6 md:px-6">
        ...
        <article className="relative flex min-h-[150px] items-center overflow-hidden rounded-xl bg-brand-blue-dark px-5 py-6 md:min-h-[210px] md:px-8">
          <div
            className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-brand-blue-light/25"
            aria-hidden="true"
          />
          <div
            className="pointer-events-none absolute -bottom-12 right-8 h-32 w-32 rounded-full bg-brand-orange/25"
            aria-hidden="true"
          />
          <div className="relative z-10 max-w-[75%] text-left">
            <div
              className="mb-3 h-6 w-16 rounded-sm border border-primary-foreground/40 md:h-8 md:w-24"
              aria-hidden="true"
            />
            <h3 className="font-display text-[22px] font-bold italic uppercase leading-[1.05] text-primary-foreground md:text-3xl">
              Crédito rápido e fácil
            </h3>
            <p className="mt-2 text-[15px] italic leading-[1.3] text-primary-foreground/85 md:text-lg">
              Soluções de crédito para realizar seus planos.
            </p>
            <div className="mt-3 flex items-center gap-4 text-primary-foreground/80">
              <span className="flex items-center gap-1 text-[12px] uppercase italic md:text-sm">
                <Zap className="h-4 w-4" aria-hidden="true" /> Aprovação ágil
              </span>
              <span className="flex items-center gap-1 text-[12px] uppercase italic md:text-sm">
                <ShieldCheck className="h-4 w-4" aria-hidden="true" /> Segurança
              </span>
            </div>
          </div>
        </article>
